...
typedef struct{
  uint16_t  BandoSvcHdle;                  /**< Bando Service Handle */
...
static BANDO_Context_t BANDO_Context;
...
static SVCCTL_EvtAckStatus_t BANDO_EventHandler(void *p_pckt);
 ...
 */
#define COPY_BANDO_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
#define COPY_SWITCH_C_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
 ...
 */
static SVCCTL_EvtAckStatus_t BANDO_EventHandler(void *p_Event)
{
  SVCCTL_EvtAckStatus_t return_value;
  hci_event_pckt *p_event_pckt;
  evt_blecore_aci *p_blecore_evt;
  aci_gatt_attribute_modified_event_rp0 *p_attribute_modified;
  BANDO_NotificationEvt_t                 notification;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          if(p_attribute_modified->Attr_Handle == (BANDO_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = BANDO_SWITCH_C_NOTIFY_DISABLED_EVT;
                BANDO_Notification(&notification);
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = BANDO_SWITCH_C_NOTIFY_ENABLED_EVT;
                BANDO_Notification(&notification);
          ...
          else if(p_attribute_modified->Attr_Handle == (BANDO_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;

            notification.EvtOpcode = BANDO_LED_C_WRITE_NO_RESP_EVT;
...
void BANDO_Init(void)
{
   ...
   */
  SVCCTL_RegisterSvcHandler(BANDO_EventHandler);

  /**
   * Bando
   *
   * Max_Attribute_Records = 1 + 2*2 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for Bando +
   *                                2 for LED_C +
   *                                2 for SWITCH_C +
  ...
  COPY_BANDO_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(BANDO_Context.BandoSvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_service command: Bando, error code: 0x%x \n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_service command: BandoSvcHdle = 0x%04X\n",BANDO_Context.BandoSvcHdle);
  }
  ...
  ret = aci_gatt_add_char(BANDO_Context.BandoSvcHdle,
                          UUID_TYPE_128,
                          (Char_UUID_t *) &uuid,
                          SizeLed_C,
  ...
  ret = aci_gatt_add_char(BANDO_Context.BandoSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(BANDO_Context.Switch_CCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
...
tBleStatus BANDO_UpdateValue(BANDO_CharOpcode_t CharOpcode, BANDO_Data_t *pData)
{
  ...
  {
    case BANDO_LED_C:
      ret = aci_gatt_update_char_value(BANDO_Context.BandoSvcHdle,
                                       BANDO_Context.Led_CCharHdle,
                                       0, /* charValOffset */
